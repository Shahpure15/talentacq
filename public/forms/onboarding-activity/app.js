const API_URL = '/api/onboarding-activities';
let currentId = null;
let debounceTimer = null;
let mode = 'new';
const fieldIds = ['activity-name', 'description', 'is-mandatory', 'is-active'];

function applyFieldSizes() {
  const c = document.getElementById('activity-code'); if(c) c.style.flex = 'none'; if(c) c.style.width = '150px';
  const m = document.getElementById('is-mandatory'); if(m) m.style.flex = 'none'; if(m) m.style.width = '80px';
...
function populateForm(data) {
  currentId = data.activity_id;
  setIdBadge(currentId);
  document.getElementById('activity-code').value = data.activity_code || '';
  document.getElementById('activity-name').value = data.activity_name || '';
  document.getElementById('description').value = data.description || '';
  document.getElementById('is-mandatory').value = data.is_mandatory != null ? data.is_mandatory : '1';
...
function resetForm(clearCode) {
  currentId = null;
  if (clearCode) document.getElementById('activity-code').value = '';
  document.getElementById('activity-name').value = '';
  document.getElementById('description').value = '';
  document.getElementById('is-mandatory').value = '1';
...
async function handleSave() {
  const payload = {
    activity_code: document.getElementById('activity-code').value.trim(),
    activity_name: document.getElementById('activity-name').value.trim(),
    description: document.getElementById('description').value.trim(),
    is_mandatory: document.getElementById('is-mandatory').value,
    is_active: document.getElementById('is-active').value
  };
  if (!payload.activity_code || !payload.activity_name) return showMessage('Code and Name required.', 'error');
  try {
    ...
    const json = await res.json();
    if (json.success) {
      if (!currentId) { resetForm(true); if (mode === 'new') loadNextId(); showMessage('Activity saved.', 'success'); }
      else showMessage('Activity updated.', 'success');

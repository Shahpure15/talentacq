const API_URL = '/api/sourcing-channels';
let currentId = null;
let debounceTimer = null;
let mode = 'new';
const fieldIds = ['channel-name', 'channel-type', 'is-active'];

function applyFieldSizes() {
  const c = document.getElementById('channel-code'); if(c) c.style.flex = 'none'; if(c) c.style.width = '150px';
...
function populateForm(data) {
  ...
  document.getElementById('channel-code').value = data.channel_code || '';
  document.getElementById('channel-name').value = data.channel_name || '';
  document.getElementById('channel-type').value = data.channel_type || '';
...
function resetForm(clearCode) {
  currentId = null;
  if (clearCode) document.getElementById('channel-code').value = '';
...
async function handleSave() {
  const payload = {
    ...
    channel_type: document.getElementById('channel-type').value.trim(),
    is_active: document.getElementById('is-active').value
  };
  if (!payload.channel_code || !payload.channel_name) return showMessage('Code and Name required.', 'error');
  try {
    ...
    const json = await res.json();
    if (json.success) {
      if (!currentId) { resetForm(true); if (mode === 'new') loadNextId(); showMessage('Channel saved.', 'success'); }
      else showMessage('Channel updated.', 'success');

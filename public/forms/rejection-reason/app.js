const API_URL = '/api/rejection-reasons';
let currentId = null;
let debounceTimer = null;
let mode = 'new';
const fieldIds = ['reason-category', 'description', 'is-active'];

function applyFieldSizes() {
  const c = document.getElementById('reason-code'); if(c) c.style.flex = 'none'; if(c) c.style.width = '150px';
...
function populateForm(data) {
  currentId = data.reason_id;
  setIdBadge(currentId);
  document.getElementById('reason-code').value = data.reason_code || '';
  document.getElementById('reason-category').value = data.reason_category || '';
...
function resetForm(clearCode) {
  currentId = null;
  if (clearCode) document.getElementById('reason-code').value = '';
  document.getElementById('reason-category').value = '';
...
async function handleSave() {
  const payload = {
    reason_code: document.getElementById('reason-code').value.trim(),
    reason_category: document.getElementById('reason-category').value.trim(),
    description: document.getElementById('description').value.trim(),
    is_active: document.getElementById('is-active').value
  };
  if (!payload.reason_code) return showMessage('Code required.', 'error');
  try {
    ...
    const json = await res.json();
    if (json.success) {
      if (!currentId) { resetForm(true); if (mode === 'new') loadNextId(); showMessage('Reason saved.', 'success'); }
      else showMessage('Reason updated.', 'success');
